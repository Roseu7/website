...
export function withSecurityHeaders(request: Request, response: Response, nonce: string): Response {
  ...
  const isContactRoute = new URL(request.url).pathname === "/contact";
  const scriptSrc = `script-src 'self' 'nonce-${nonce}'${isMapDocument ? " 'unsafe-eval'" : ""}`;
  const turnstileSource = isContactRoute ? " https://challenges.cloudflare.com" : "";
  const csp = [
    "default-src 'self'",
    "base-uri 'self'",
    "frame-ancestors 'none'",
    "object-src 'none'",
    `${scriptSrc}${turnstileSource}`,
    `frame-src 'self'${turnstileSource}`,
    "worker-src 'self' blob:",
    "style-src 'self' 'unsafe-inline'",
    "img-src 'self' data: https:",
    "font-src 'self'",
    `connect-src 'self'${turnstileSource} https://home.roseu.net https://roseu.net https://www.roseu.net https://roseu.cloudflareaccess.com ws: wss:`,
    "form-action 'self'",
