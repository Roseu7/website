...
import type { HomeControlActionResult, PcPowerState, PcStatePayload } from "~/utils/home-control";
...
interface HomeControlEnv {
  ...
  WAKE_SERVICE_BEARER_TOKEN?: string;
...
export async function fetchPcState(context: AppLoadContext): Promise<PcStatePayload> {
  ...
  try {
    const response = await fetchWithTimeout(targetUrl, { method: "GET", headers });
    if (!response.ok) {
      return {
        power: response.status === 404 ? "unknown" : "off",
        canWake: Boolean(env.WAKE_SERVICE_URL),
        canShutdown: false,
        source: "derived",
        checkedAt,
