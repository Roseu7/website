...
export async function createWhitelistApplication(
  ...
  const [existing, approved, localTrust, crossTrust, crossDenied, recent] = await Promise.all([
    ...
    db.prepare("SELECT COUNT(*) AS count FROM whitelist_applications WHERE discord_id = ? AND created_at >= ?")
      .bind(input.discordId, rateLimitSince).first<{ count: number }>(),
  ]);
  ...
  const allowApprovedRetry = localTrust?.state === "manual_required";
  const now = new Date().toISOString();
  const result = await db.prepare(
    ...
    SELECT ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0, ?, ?
    WHERE (SELECT COUNT(*) FROM whitelist_applications WHERE discord_id = ? AND created_at >= ?) < 5
      AND NOT EXISTS (
        SELECT 1 FROM whitelist_applications
        WHERE server_id = ? AND minecraft_uuid = ? AND status = 'pending'
      )
      AND (
        ? = 1
        OR NOT EXISTS (
          SELECT 1 FROM whitelist_applications
          WHERE server_id = ? AND minecraft_uuid = ? AND status = 'approved'
        )
      )
    RETURNING id`
  ).bind(
    ...
    input.discordId, rateLimitSince,
    input.serverId, input.minecraftUuid,
    allowApprovedRetry ? 1 : 0,
    input.serverId, input.minecraftUuid,
  ).first<{ id: number }>();
  if (!result) {
    const [latestRecent, latestPending, latestApproved] = await Promise.all([
      db.prepare("SELECT COUNT(*) AS count FROM whitelist_applications WHERE discord_id = ? AND created_at >= ?")
        .bind(input.discordId, rateLimitSince).first<{ count: number }>(),
      db.prepare("SELECT 1 FROM whitelist_applications WHERE server_id = ? AND minecraft_uuid = ? AND status = 'pending' LIMIT 1")
        .bind(input.serverId, input.minecraftUuid).first(),
    ...
    if ((latestRecent?.count ?? 0) >= 5) {
      return { ok: false as const, message: "短時間の申請回数が上限に達しています。" };
    }
    if (latestPending) return { ok: false as const, message: "このMinecraftアカウントは既に申請中です。" };
    if (latestApproved && !allowApprovedRetry) {
      return { ok: false as const, message: "このMinecraftアカウントは既に承認されています。" };
    }
    return { ok: false as const, message: "申請を処理できませんでした。少し時間を空けてください。" };
  }
  await db.prepare("UPDATE invite_codes SET used_count = used_count + 1 WHERE code = ? AND server_id = ?")
    .bind(code, input.serverId).run();
  return { ok: true as const, id: Number(result.id), status };
}
